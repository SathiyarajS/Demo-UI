...
import { Plus, Heart, UserCheck, Clock, ChevronRight, Calendar, TrendingUp, Users, MessageSquare, BarChart3, Sparkles, Globe, Camera, Search } from 'lucide-react';
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-40 space-y-6">
        <Clock size={48} className="text-slate-200 animate-spin" />
        <p className="text-slate-400 font-bold uppercase tracking-[0.3em] text-xs">Loading Portfolio...</p>
      </div>
    );
  }

  if (MOCK_WEDDINGS.length === 0) {
    return (
      <EmptyState 
        icon={Search}
        title="No Weddings Found"
        description="You haven't created any wedding projects yet. Start by creating your first one!"
        actionLabel="Create New Wedding"
        onAction={() => setCurrentScreen('create-wedding')}
      />
    );
  }
  ...
  return (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
      <div className="lg:col-span-2 space-y-10">
        ...
        <Card className="p-10 space-y-8 border-none shadow-xl bg-indigo-600 text-white overflow-hidden relative">
          <div className="relative z-10 space-y-6">
            <div className="space-y-2">
              <h2 className="text-3xl font-bold tracking-tight">Sales & Marketing Kit</h2>
              <p className="text-indigo-100 opacity-80">Everything you need to pitch to new couples and photographers.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-6 bg-white/10 rounded-3xl border border-white/10 space-y-4 hover:bg-white/20 transition-all cursor-pointer group">
                <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center">
                  <Globe size={24} />
                </div>
                <div>
                  <h3 className="font-bold text-lg">Demo Website</h3>
                  <p className="text-xs text-indigo-200">Share a live demo of the guest experience.</p>
                </div>
                <button className="text-xs font-bold flex items-center gap-2 group-hover:translate-x-1 transition-transform">
                  Copy Link <ChevronRight size={14} />
                </button>
              </div>
              <div className="p-6 bg-white/10 rounded-3xl border border-white/10 space-y-4 hover:bg-white/20 transition-all cursor-pointer group">
                <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center">
                ...
                <div>
                  <h3 className="font-bold text-lg">Photographer Pitch</h3>
                  <p className="text-xs text-indigo-200">Showcase the AI matching & delivery workflow.</p>
                </div>
                <button className="text-xs font-bold flex items-center gap-2 group-hover:translate-x-1 transition-transform">
