...
import { Globe, Sparkles, Palette, Image as ImageIcon, ChevronRight, Heart, Star, CheckCircle2, X, Download, Share2, Home, Calendar } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
      <div className="lg:col-span-2 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <Card className="p-8 space-y-6 hover:translate-y-[-4px] transition-all duration-500 border-none shadow-xl">
            <div className="flex justify-between items-start">
              <div className="w-14 h-14 rounded-2xl bg-blue-50 text-blue-500 flex items-center justify-center shadow-lg shadow-blue-500/10">
                <ImageIcon size={28} />
              </div>
              <Badge variant="info">New Photos</Badge>
            </div>
            <div className="space-y-2">
              <h3 className="text-xl font-bold tracking-tight">Engagement Gallery</h3>
              <p className="text-slate-500 font-medium leading-relaxed">42 professional photos ready for your review and selection.</p>
            </div>
            <button onClick={() => setActiveTab('gallery')} className="text-sm font-bold text-slate-900 hover:underline flex items-center gap-2 group">
              Open Gallery <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </button>
          </Card>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <Card className="p-10 space-y-8 border-none shadow-xl">
          ...
          <div className="space-y-6">
            {[
              { title: 'Catering Menu Finalization', category: 'Food & Drink', deadline: '2 days left', desc: 'Please review the final menu for the main reception and cocktail hour.' },
              { title: 'Floral Arrangement Layout', category: 'Decor', deadline: '5 days left', desc: 'Review the floor plan for the ceremony floral pillars and aisle markers.' },
              { title: 'Music Selection for First Dance', category: 'Entertainment', deadline: '1 week left', desc: 'Confirm your choice for the first dance and parent dances.' }
            ].map((item, i) => (
              <div key={i} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 space-y-4 group hover:border-slate-900 transition-all cursor-pointer">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button className="flex-1 btn-primary py-3 text-xs font-bold">Approve Now</button>
                </div>
              </div>
            ))}
          </div>
